<article id="post-<?php the_ID(); ?>" <?php post_class('row'); ?>>
    ...
    <div class="container">
        ...
        if($price && floatval($price) != 0 && is_float(floatval($price))) : ?>
            ...
            <button form="reservation-form" type="submit" class="btn btn-outline-secondary btn-lg pull-right"><?php esc_html_e( 'Reserve', 'imelab' ); ?><br><?php echo $price."€"?></button>
        
        <?php
        elseif($d && $d->format($format) == $date) : ?>
            <button type="button" class="btn btn-outline-secondary btn-lg pull-right" disabled><?php esc_html_e('Available from', 'imelab'); ?><br><?php echo $date ?></button>
        <?php
        endif; ?>
    </div>
</article>